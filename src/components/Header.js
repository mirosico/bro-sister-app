        ...
        <div className="collapse navbar-collapse justify-content-end" id="navbarNavAltMarkup">
          {auth().currentUser
            ? <div className="navbar-nav">
              <Link className="nav-item nav-link mr-3 email" to="/chat">{auth().currentUser.email}</Link>
              <button className="btn btn-primary mr-3 no-top-margin" onClick={() => auth().signOut()}>Logout</button>
            </div>
            : <div className="navbar-nav">
